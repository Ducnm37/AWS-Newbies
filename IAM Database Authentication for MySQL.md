...
Mục đích của việc này là setup 1 database chỉ cho phép các user được gắn role access vào RDS mới có thể truy cập vào DB bằng cách gen token, khi không được gắn role thì sẽ không thể gen token và không truy cập được vào database. Chỉ có tác dụng với các server trên hệ thống AWS VPC, không sử dụng cho server ngoài
